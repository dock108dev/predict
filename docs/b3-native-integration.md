...
| Fourth slot: ProphetX recommended, unselected | Provisional bounded REST producer using existing adapter | Adapter tests plus actual retained sandbox quotes projected as unsized dated observations | Earlier Slice 3 only; no new sandbox access | **None** | Historical sandbox quote projection; no newly qualified ordinary production overlap | Owner selection, production provisioning/tournament scope, quantity/value ownership, source timestamps, fees/rules; no current stream-delivery qualification |
...
Await fourth-venue selection and Novig provisioning references, then finalize and request approval of the **single** linked qualification proposal. Enter secrets locally, never in chat. No provider message is authorized. If one venue remains unavailable, retain its blocked row while continuing independent engineering. Do not mark B3 complete on fixture or sandbox success.

Further independent work: finish native listing-period/rule association and production tournament selection from available documentation/retained data; extend focused source-scope fixtures where native semantics are known. B4 owns model/Pinnacle acquisition; B5 owns broader canonical sport/period/futures and economic support. No commits, pushes, trading, spending or publication.

B3 native REST currently uses the shared flat journal and explicitly rejects segmented selection. B2's existing segmented lifecycle/history path remains supported and separately regression-tested; extending native REST verification across segmented finalization is independent engineering still available. No unverified native segmented package is presented as complete.
