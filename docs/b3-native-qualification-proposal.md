# B3 bounded native qualification proposal — awaiting access decisions
...
The implementation is reviewable. Finalizing this one proposal requires the owner's fourth-venue selection and Novig environment/provisioning references. ProphetX is recommended, not selected. No approval is inferred from the implementation request. Do not create an approved authorization file until the owner approves the completed scope below; no credentials in chat.

## Proposed single production session

- Sources: Kalshi, Polymarket **US**, Novig and **ProphetX only if selected**. A different fourth venue requires revising this proposal before approval, not substituting a source during execution.
...
After selection/access references are supplied, engineering freezes the UTC window, exact production tournament/native scope, run spec, app identity and isolated output into one reviewable authorization package. `native_approval.py` checks those hashes and once-only consumption; `app.dashboard.native_preview` opens the ordinary dashboard idle. Start stays disabled without a matching approved package. This document is the bounded proposal, not that approval file.

Owner action now: select ProphetX or name the fourth venue; state whether Novig production/QA provisioning exists and give only the documented secret-store reference and issued endpoint/access-document location. If ProphetX is selected, identify available production access and its documented local reference. **No new authenticated access or collection has occurred.**
