# B6 finite offline integrated workload

Authorized by the owner's attached B6 request, September 22, 2026. Fixture-only;
no real source activation, credentials, credits or existing beta restart.

Executable: `.venv/bin/python -m tests.b6_integrated evidence/b6-offline-20260923/timed-final 180 10`.
...
Each segment also supplies one B5 reviewed synthetic market through the ordinary
...
annotated B5 inputs are explicitly distinguished from native wire parsing.
...
REST polls every 30 seconds, discovery every 60 seconds. B5 book images at each
