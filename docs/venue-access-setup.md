...
September 21, 2026. Setup stores secrets locally; it does not start collection. Venue integration IN PROGRESS; beta NOT READY. The owner has sent both access requests; the steps below are retained setup guidance, not instructions to send duplicates.
...
Enter the client ID and secret at the hidden prompts. For QA, use `--environment qa`. This saves JSON under `prediction-arb.novig.production` / `market-data` (or the QA equivalent). The older diagnostic setup command stored separate fields and is not the venue integration setup command.

GraphQL is useful for a small discovery/display bridge only. It does not currently supply qualified purchase comparisons. No key is needed for the [separately proposed public sample](novig-graphql-sample-proposal.md); it has not been approved or run. NBX remains the intended durable route.
