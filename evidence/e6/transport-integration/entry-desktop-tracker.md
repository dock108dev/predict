...
Updated: September 15, 2026 (E6 bounded synthetic collection complete; real-feed qualification remains separate)
...
## Current direction — our price and net opportunities

Build a personal prediction-market pricing and opportunity platform. **External sportsbook/exchange lines and sports data are reference inputs only. Supported prediction markets are the only potential execution venues.** Reference ingestion and our-price estimation belong in the first expanded delivery alongside arbitrage.
...
**First expanded delivery:** one NFL pregame moneyline event, one usable external reference source, existing Kalshi/Polymarket US books, our price, net Arb/Mispricing results, a readable market-watch screen and reproducible saved history. Broader venue coverage, learned weights, lead/lag validation and maker simulations follow. The four-signal data model belongs in E1; E5 need not implement E7/E9 behavior or fill the interface with unfinished features.
...
**One concrete next handoff:** [prepare the bounded real-feed transport integration](/Users/michaelfuscoletti/Desktop/prediction-arb/docs/e6-real-feed-handoff.md), resolving the transport, entitlement, exact event, duration/cadence and cost specification before any real request. **Do not launch that step.** The deferred 250 ms gate stays deferred. To reproduce the isolated synthetic preview: `/Users/michaelfuscoletti/Desktop/prediction-arb/scripts/e6-preview start`; it starts idle. The same script provides `stop` and `cleanup`; full instructions are in the report.
...
| E6 | First bounded synthetic collection/recovery/reopen package complete. Full real-feed useful-duration qualification remains separate. | Existing collector stability; reference side uses E2. Can overlap E3–E5; practical owner loop and agreed collection/recovery evidence. |
...
Delivery path: **E1 → E2 → E3 → E4 → E5**, with **E6 alongside**; then E7/E8, E9 and evidence-led E10 expansion. A provider-specific access limitation blocks its live verification, not independent contracts, fixtures or UI work. The Odds API is recommended for fixture implementation; no source is live-qualified or purchased.
...
- [ ] **Reference/pricing foundation (E1–E3):** one reference source, exact event/market matching, source-family provenance, our-price baseline and deterministic replay.
