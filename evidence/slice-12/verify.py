"""Reproducible verification; writes only Slice 12 evidence and project test DBs."""
import hashlib,json,pathlib,subprocess,sys
root=pathlib.Path(__file__).resolve().parents[2]; out=root/'evidence/slice-12'
commands=[('offline-tests',[sys.executable,'-m','unittest','discover','-s','tests','-v']),
...
(out/'preservation.json').write_text(json.dumps(dict(checked=len(before),changed=changed,plan_unchanged='PLAN.md' not in changed,historical_evidence_unchanged=not any(p.startswith('evidence/') for p in changed),credentials='Not read, hashed or changed.'),indent=2)+'\n')
