...
class Runner:
 ...
 def validate_child(self, decision, parents):
  import re
  url=decision.get('url','');source=decision.get('source');parent=decision.get('parent_url');topic=decision.get('topic')
  if source not in self.spec['child_hosts'] or parent not in parents or parents[parent][0]!=source or topic not in self.spec['topics']:raise GlobalStop('Unresolved source/topic/parent')
  raw=parents[parent][1].decode('utf-8');parsed=urlsplit(url)
  # Only complete directly printed absolute links, never guessed paths or search results.
  linked=('('+url+')' in raw or 'href="'+url+'"' in raw or "href='"+url+"'" in raw)
  if not linked or parsed.scheme!='https' or parsed.hostname not in self.spec['child_hosts'][source] or parsed.username or parsed.password or parsed.port not in (None,443) or parsed.query or parsed.fragment:raise GlobalStop('Child link outside frozen discovery scope')
  path=parsed.path.lower()
  if path.endswith('.pdf') or any(x in path for x in ('/api-reference/markets/','/markets/','/events/','/search','proxy','mirror','fee-schedule.pdf')):raise GlobalStop('Excluded child endpoint')
  passage=decision.get('relevance_passage','')
  if not passage or passage not in raw or url not in passage:raise GlobalStop('Missing verbatim relevance evidence')
  return dict(source=source,url=url,format='text' if path.endswith(('.md','.txt')) else 'html')
 async def run(self):
  parents={}
  for row in self.spec['requests']:
   if row['source'] not in self.closed:
    data=await self.get(row)
    if data and row['format'] in ('html','text'):parents[row['url']]=(row['source'],data)
  if not parents:
   self.reason='No successful document parents; stop discovery';return
  print('Supply one JSON list of at most three Kalshi and two US direct official document links, or [] to leave slots unused. Each needs source, parent_url, url, topic and verbatim relevance_passage. Clock continues.',flush=True)
  while not select.select([sys.stdin],[],[],0)[0]:self.check();await asyncio.sleep(.05)
  decisions=json.loads(sys.stdin.readline());save('discovery-decisions.json',decisions)
  if not isinstance(decisions,list) or len(decisions)>5:raise GlobalStop('Discovery request cap')
  counts={};rows=[]
  for decision in decisions:
   row=self.validate_child(decision,parents);source=row['source'];counts[source]=counts.get(source,0)+1
   if counts[source]>(3 if source=='kalshi' else 2):raise GlobalStop('Per-source discovery cap')
   rows.append(row)
  for row in rows:
