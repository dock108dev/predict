...
runs=json.loads((BASE/'final-candidate/measurements.json').read_text())
...
summary=dict(complete=all(all(r['checks'].values()) for r in rows+fault_rows),
    source_identity_sha256=sha256((BASE/'final-candidate/source-identity.json').read_bytes()).hexdigest(),
