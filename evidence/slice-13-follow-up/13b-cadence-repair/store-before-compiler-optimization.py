...
class Store:
    def __init__(self, connection):
        self.db=connection;self.tree_cache=OrderedDict();self.tree_cache_bytes=0;self.precompiled={}
    ...
    def put(self, value):
        ...
        for h,body in artifacts.items():self.immutable('artifact',dict(hash=h,content=body,kind='tree'),['hash'])
        for parent,child in edges:self.immutable('artifact_edge',dict(parent=parent,child=child),['parent','child'])
        return root

    def compile(self, value):
        """Compile identical immutable subtrees once; always verify durable writes.

        Cache holds compiled bytes only, never claims a transaction committed.
        Rollback therefore cannot poison artifact persistence on the next call.
        """
        artifacts={};edges=set()
        def node(v,root=False):
            cachekey=encoded(v) if isinstance(v,(dict,list,tuple)) else None
            cached=self.tree_cache.get(cachekey) if cachekey is not None else None
            if cached is not None:
                data,refs,blobs,links,_=cached
                artifacts.update(blobs);edges.update(links)
                self.tree_cache.move_to_end(cachekey)
            else:
                refs=[]
                if isinstance(v,dict):
                    parts=[]
                    for k,x in sorted(v.items()):
                        a,r=node(x);parts.append([k,a]);refs.extend(r)
                    data=['dict',parts]
                elif isinstance(v,(list,tuple)):
                    parts=[]
                    for x in v:
                        a,r=node(x);parts.append(a);refs.extend(r)
                    data=['list',parts]
                else:data=['value',v]
                body=encoded(data)
                if len(body)>4096:
                    h=sha256(body).hexdigest();artifacts[h]=body
                    edges.update((h,child) for child in set(refs))
                    data=['ref',h];refs=[h]
                # Include the entire dependency closure even when already present
                # in this call (a cache entry must be independently reusable).
                if cachekey is not None and len(cachekey)>=2048:
                    def closure(h,found):
                        if h in found:return
                        found[h]=artifacts[h]
                        for parent,child in edges:
                            if parent==h:closure(child,found)
                    blobs={}
                    for h in refs:closure(h,blobs)
                    links={e for e in edges if e[0] in blobs}
                    from app.dashboard.bounds import retained_bytes
                    size=retained_bytes((cachekey,data,refs,blobs,links))+256
                    if size<=4*1024*1024:
                        while self.tree_cache and self.tree_cache_bytes+size>4*1024*1024:
                            _,old=self.tree_cache.popitem(last=False);self.tree_cache_bytes-=old[4]
                        self.tree_cache[cachekey]=(data,refs,blobs,links,size)
                        self.tree_cache_bytes+=size
            if root and data[0]!='ref':
                body=encoded(data);h=sha256(body).hexdigest();artifacts[h]=body
                edges.update((h,child) for child in set(refs));return ['ref',h],[h]
            return data,refs
        root=node(value,True)[0][1]
        return root,artifacts,edges
    ...
    def active(self, sid):
        row=self.db.execute('SELECT * FROM capture_session WHERE id=%s FOR UPDATE',(sid,)).fetchone()
