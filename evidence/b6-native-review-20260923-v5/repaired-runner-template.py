...
class Runner:
 ...
 async def get(self,row):
  ...
  try:
   async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=min(lim['request_seconds'],lim['seconds']-(time.monotonic()-self.start)),ceil_threshold=float('inf')),trust_env=False,auto_decompress=False,connector=aiohttp.TCPConnector(force_close=True),headers={'Accept-Encoding':'identity'}) as session:
