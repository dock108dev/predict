...
root=pathlib.Path.cwd(); out=root/'evidence/slice-9'; results=[]
commands=[('tests',[sys.executable,'-m','unittest','discover','-s','tests','-v'])]
commands += [(name,[sys.executable,'-m','app.'+name]) for name in ['example','polymarket_us_example','prophetx_example','kalshi_example','novig_example','normalization_example','matching_example','moneyline_example','fee_example']]
