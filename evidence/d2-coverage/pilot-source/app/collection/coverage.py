...
def market_record(venue, page, body, native, eid, index):
    ...
    try:
        ...
        elif market.state.value != 'active':
...
def catalog(pages, venue, as_of):
    ...
    # Those pages supersede embedded market copies, avoiding false duplicate
    # conflicts caused by metadata changing between the two HTTP receipts.
    ...
    for page, body, data in event_pages:
        for native in data['events']:
            ...
            if venue == 'polymarket_us':
                ...
                for m in (embedded or []) if str(native.get('gameId')) not in independent_games else []:
                    row_index += 1
