...
class Discovery:
    ...
    def receipt(self, venue, row):
        ...
        self.session.emit(venue, dict(row, discovery_generation=self.generation))
    ...
    async def discover(self, force=False):
        # Concurrent initial producer calls share one traversal; refresh never overlaps.
        async with self.lock:
            ...
            try:
                ...
                errors = [type(e).__name__+':'+str(e) for e in results if isinstance(e, Exception)]
                error = '; '.join(errors) if errors else None
            finally:
                # Keep partial discovery on cancellation/limits too. No fresh requests here.
                ...
                for v, cat in cats.items():
                    ...
                    for row in cat['markets']:
                        ...
                        try:
                            ...
                            row['subscription_exclusion'] = 'market_parse_error'
                ...
                self.completed = True
                s = self.session
                s.emit('session', dict(type='coverage_inventory', generation=self.generation,
                    inventory=cats, discovery_error=error,
                    previous_generation=self.generation-1 if previous else None))
                self.coverage = cats
            if error:
...
class Venue:
    ...
    def snapshot(self):
        ...
        return dict(eligible=self.eligible, selected=len(self.selected), **counts,
    ...
    async def reconcile(self):
        inventory = self.session.discovery.inventory[self.venue]
        ids, self.eligible = select_inventory(inventory, now())
        ids = [m for m in ids if m in self.session.discovery.markets[self.venue]]
        self.selected = ids
        ...
        for name, group in list(self.groups.items()):
            if group['ids'] not in desired or group['signature']!=signatures.get(group['ids']):
...
class ContinuousSession(TransportSession):
    ...
    def status_coverage(self):
        ...
        return {v:dict(discovered_events=len(cats.get(v,{}).get('events',[])),
                       discovered_markets=len(cats.get(v,{}).get('markets',[])),
