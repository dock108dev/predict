...
class Inventory(unittest.TestCase):
    def test_independent_market_pages_and_missing_gameid(self):
        ...
        self.assertEqual(us['market_completeness'],'exhausted_for_retained_events')
        self.assertEqual([m['id'] for m in us['markets']],['independent'])
    ...
    def test_connection_and_ingress_resource_stop(self):
        reasons=[]
        s=SimpleNamespace(spec=spec(),connection_attempts=12,request_stop=reasons.append)
...
class Lifecycle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_refresh_new_departed_closed_rescheduled_and_cutoff(self):
        ...
        s=SimpleNamespace(spec=spec(),credentials={},health={},stop_event=stop,connection_attempts=0,
