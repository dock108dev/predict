...
from tests.test_b5_nfl_resolution import fixture,rec
...
def sequence(s,g,e):
    ...
    c=rec(s,g,e,minute=2,supersedes=[a['id']])
    d=rec(s,g,e,'venue',minute=3,supersedes=[b['id']])
    f=rec(s,g,e,minute=4,home=17)
    corrected=rec(s,g,e,minute=5,home=10,supersedes=[c['id'],f['id']])
    return [a,b,c,d,f,corrected]


class Session(BaseSession):
    prefix='b5-nfl-resolution-'
    async def start(self):
        self.journal=ObservationJournal(self.output/(self.sid+'.jsonl'));self.state='running';self.queue=asyncio.Queue()
        rows,_,_,e=fixture('moneyline','first_half')
        for row in rows:
...
if __name__=='__main__':
    import sys
    web.run_app(create_app(owner=owner(Path(sys.argv[1])),sessions={}),host='127.0.0.1',port=8811,access_log=None)
