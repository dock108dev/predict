"""Transport-free MLB fixture in the ordinary UI. Inherits real owner Stop/cutoffs.
...
from tests.test_b5_mlb import fixture,reference
...
class Session:
    ...
    def __init__(self,spec,output,endpoints):
        self.sid=self.prefix+str(uuid4());self.spec=spec;self.output=output;self.projection=SessionProjection()
    ...
    async def start(self):
        ...
        rows=self.fixture();now=datetime.now(timezone.utc).isoformat()
        for row in rows:
        ...
        for ref in [self.reference(g,at=now),self.reference(g,kind='rating',at=now),self.reference(g,semantics=False,at=now)]:
...
def owner(root,session_factory=Session):
    def config():
    ...
    return Owner(root/'legacy',pilot_output=root/'sessions',product_mode=True,spec_factory=config,session_factory=session_factory,endpoints=endpoints)

if __name__=='__main__':
    import sys
    web.run_app(create_app(owner=owner(Path(sys.argv[1])),sessions={}),host='127.0.0.1',port=8798,access_log=None)
