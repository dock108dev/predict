...
from tests.test_b5_ncaab_first_half import fixture,reference
from tests.test_b5_score_lines import fixture as full_fixture,reference as full_reference
...
def combined_fixture():
    base=None
    for family,line,period in [('moneyline',None,'first_half'),('spread','-3','first_half'),('total','70.5','first_half'),('spread','-3','full_game'),('moneyline',None,'unsupported')]:
        rows=full_fixture('NCAAB') if period=='full_game' else fixture(family=family,line=line)
        ...
        if period=='unsupported':
            for c in rows[1]['inventory'].values():c['markets'][0]['score_review']['descriptor']['winner_structure']='fractional_tie_0.50'
...
if __name__=='__main__':
    import sys
    web.run_app(create_app(owner=owner(Path(sys.argv[1])),sessions={}),host='127.0.0.1',port=8810,access_log=None)
