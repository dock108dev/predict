"""Finite, explicit offline workload. No production launcher imports this file.
...
from tests.native_fixture import factory as novig_factory
from tests.test_native_integration import configuration as base_configuration
...
def case_rows(case):
    ...
    if period=='first_half':
        import importlib
        return importlib.import_module('tests.test_'+sport.lower()+'_first_half').fixture(family='total',line='20.5' if sport.endswith('F') or sport=='NFL' else '100.5')
    if period in ('first_5','period_2'):
        from tests.test_periods import fixture
        return fixture(sport,period,'moneyline')
    if sport in ('MLB','NHL'):
        from tests.test_diamond_ice_resolution import fixture
        return fixture(sport)[0]
    from tests.test_score_lines import fixture
            ...
            def emit(self,source,record):
                if record['type']=='coverage_inventory':
                    ...
                    for v,c in runtime.extra[1]['inventory'].items():
                        ...
                        # Keep subscription telemetry native-only. The inputs
    ...
    async def annotated_books(self,metadata=False):
        ...
        for original in self.extra[2:]:
            ...
            if row['type']=='prediction_book':
                # older transport-free fixtures rely on listing-state
