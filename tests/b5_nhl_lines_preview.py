...
from tests.test_b5_nhl_lines import fixture,reference


def combined_fixture():
    nba=fixture();college=fixture('NHL','total','5.5')
    ...
    nba+=college[2:]
    # An explicitly labeled unsupported score convention remains visible.
    gap=fixture()
    text=json.dumps(gap).replace('SYNTHETIC-NHL-game-1','SYNTHETIC-NHL-shootout-score-gap')
    replacements={v[k]:v[k]+'-score-gap' for c in gap[1]['inventory'].values() for kind,k in [('events','id'),('markets','id')] for v in c[kind]}
    for old,new in sorted(replacements.items(),key=lambda x:-len(x[0])):text=text.replace(old,new)
    gap=json.loads(text)
    # A different date is a separate game, not a conflicting duplicate identity.
    from app.normalization.nhl_lines import event_key,EVENT_FIELDS
    for source,c in gap[1]['inventory'].items():
        e=c['events'][0];e.update(original_start='2026-10-15T23:00:00Z',scheduled_start='2026-10-15T23:00:00Z',title='SYNTHETIC NHL shootout-score unsupported example')
        r=c['markets'][0]['score_review'];r['event_binding']=event_key(e);r['descriptor']['settlement_score']='unknown'
        meta=next(v['market'] for v in gap if v['type']=='market_selected' and v['source']==source)
        native=json.loads(meta['raw']['json_text']);native['SYNTHETIC_nhl_event']={k:e[k] for k in EVENT_FIELDS};meta['raw']['json_text']=json.dumps(native)
        reseal(gap,source)
...
if __name__=='__main__':
    import sys
    web.run_app(create_app(owner=owner(Path(sys.argv[1])),sessions={}),host='127.0.0.1',port=8806,access_log=None)
