...
function renderWatch(){
 ...
 $('mode').textContent=data.recovery?'Historical — Interrupted · '+(data.provenance==='mock'?'local mock test':'captured real feeds'):(live?'Live — captured real feeds':'Historical — captured from real feeds');if($('event-title'))$('event-title').textContent=data.event;
 $('notice').textContent=data.recovery?'Interrupted history: last verified records only. Shutdown and missing deliveries are unknown. No current connections or executable economics.':live?'Current observations only. A source without a synchronized recent book has no usable current quotes. No execution or qualified economics.':'Collection stopped for this saved session. Nothing shown is currently live or eligible for execution.';
 ...
 $('counts').innerHTML=[['frames','Raw frames'],['books','Book states'],['packets','Quote packets'],['ingress',data.recovery?'Verified-prefix ingress records':'Persisted ingress records']].map(([k,l])=>`<div><strong>${data.counts[k]}</strong><span>${l}</span></div>`).join('');
 $('coverage-limit').textContent=data.coverage.limitation;$('completeness').textContent=data.coverage.completeness;
 $('verification').textContent=live?'Journal-backed current view; exact native replay runs after shutdown.':`Source identity, verified prefix and native replay revalidated. ${data.replay.total_books} book states / ${data.replay.total_packets} packets; includes ${data.replay.derived_health_books} derived health states.`;
 $('transitions').textContent=data.transitions.map(t=>`${t.at} · ${t.source} · ${t.state||t.type}`).join('\n');
 $('session-id').textContent=data.session;$('chain').textContent=data.hash;if(data.recovery)$('verification').textContent+=` Verified chain: ${data.recovery.verified_chain}; last verified offset: ${data.recovery.verified_offset}; excluded bytes: ${data.recovery.excluded_trailing_bytes}.`;
...
async function refresh(){
 ...
 try{
  const nextState=await request('/api/status');if(actionPending)return;ownerState=nextState;$('owner-state').textContent=ownerState.active?ownerState.state:'Read only · no active feeds';
  ...
  $('owner-message').textContent=ownerState.error||(ownerState.active?`${ownerState.state} · ${ownerState.persisted}/${ownerState.delivered} accepted records drained`:'Read-only recovery. No collection or automatic resume.');
  const selected=$('saved').value;$('saved').innerHTML='<option value="">Current / idle</option>'+ownerState.saved.map(s=>`<option value="${s}">${s} — ${ownerState.saved_status?.[s]||"Completed"}</option>`).join('');$('saved').value=selected;
